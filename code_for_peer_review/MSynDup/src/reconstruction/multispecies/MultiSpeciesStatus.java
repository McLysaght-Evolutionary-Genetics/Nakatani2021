...
public class MultiSpeciesStatus{
	...
	Template template_;
	int maxGroup_;
	int[] group_;// 0:unmapped, 1,...,6: chromosomes
			...
			switch(template_){
				...
				case PRE2R_FUSION:
					switch(group1){
						case 1:
						case 2:
							if(group2==5||group2==6) inRedRegion = false;
							break;
						case 5:
						case 6:
							if(group2==1||group2==2) inRedRegion = false;
							break;
					}
					break;
				case POST2R_FUSION:
					switch(group1){
						...
						case 2:
						case 3:
							if(group2==5||group2==6||group2==7) inRedRegion = false;
							break;
						case 5:
						case 6:
						case 7:
							if(group2==1||group2==2||group2==3) inRedRegion = false;
							break;
					}
					break;
				case PRE2R_DOUBLE_FUSION:
					switch(group1){
						...
						case 2:
							if(group2==5||group2==6) inRedRegion = false;
							if(group2==7||group2==8) inRedRegion = false;
							break;
						case 3:
						case 4:
							if(group2==7||group2==8) inRedRegion = false;
						...
						case 8:
							if(group2==1||group2==2) inRedRegion = false;
							if(group2==3||group2==4) inRedRegion = false;
							break;
					}
					break;
			}
	...
	private boolean isValidStatus(int nextGroup){
		switch(template_){
			case FREE_FORM:
				if(nextGroup>this.maxGroup_+1) return false;
				for(int i = 1; i<nextGroup; ++i){
					if(group2blocks_.get(Integer.valueOf(i))==null) return false;
				}
				break;
			case FOUR:
				if(nextGroup>4) return false;
				for(int i = 1; i<nextGroup; ++i){
					if(group2blocks_.get(Integer.valueOf(i))==null) return false;
				}
				break;
			case SIX:
				if(nextGroup>6) return false;
				for(int i = 1; i<nextGroup; ++i){
					if(group2blocks_.get(Integer.valueOf(i))==null) return false;
				}
				break;
			case PRE2R_FUSION:
				...
				switch(nextGroup){
					case 2:
						if(group2blocks_.get(Integer.valueOf(1))==null) return false;
						break;
					case 4:
						if(group2blocks_.get(Integer.valueOf(3))==null) return false;
						break;
					case 5:
						if(group2blocks_.get(Integer.valueOf(1))==null) return false;
						break;
					case 6:
						if(group2blocks_.get(Integer.valueOf(5))==null) return false;
						break;
				}
				break;
			case POST2R_FUSION:
				if(nextGroup>7) return false;
				switch(nextGroup){
					case 2:
					case 3:
						for(int i = 1; i<nextGroup; ++i){
			...
			case PRE2R_DOUBLE_FUSION:
				...
				switch(nextGroup){
					...
					case 7:
						...
						break;
					case 8:
						if(group2blocks_.get(Integer.valueOf(7))==null) return false;
						break;
				}
				break;
		}
				...
				for(int i = 0; i<=currentIndex_; ++i){
					...
					if(isRedRegion(nextGroup, g)==false){
						for(String seg: segmentList_.get(i)){
							...
							if(significantParalogonPairs_.contains(pair)){ return false; }
						}
					}
				}
			}
			...
			for(String nextSegName: segList){
				for(int i = 0; i<=currentIndex_; ++i){
					...
					if(nextGroup!=g){
						for(String seg: segmentList_.get(i)){
							String pair = seg+"\t"+nextSegName;
							if(significantOrthologousSegmentPairs_.get(pair)!=null){ return false; }
						}
					}
				}
			}
	...
	public ArrayList<MultiSpeciesStatus> getNextStatusList(boolean checkSignificanceConstraint){
		...
		for(int g = 1;; ++g){
			...
			switch(template_){
				case FREE_FORM:
					if(g>this.maxGroup_+1) finish = true;
					break;
				case FOUR:
					if(g>4) finish = true;
					break;
			...
			if(finish) break;
			if(isValidStatus(g)==false) continue;
			if(checkSignificanceConstraint&&checkSignificanceConstraint(g)==false) continue;
	...
	public void setNextGroup(int nextGroup){
		++currentIndex_;
		if(nextGroup>maxGroup_) maxGroup_ = nextGroup;
		...
		for(int i = newSegIndex; i<flatSegmentList.size(); ++i){
			...
			for(int j = 0; j<=i; ++j){
				...
				if(speciesIndex1.intValue()==speciesIndex2.intValue()){ // update paralog distribution significance
				...
				}else{ // update ortholog distribution significance
	...
	public boolean isCompleted(){
		if(currentIndex_<segmentList_.size()-1) return false;
		return true;
	}
}
