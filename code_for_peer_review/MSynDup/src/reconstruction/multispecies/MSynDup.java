...
public class MSynDup{
	...
	static TreeMap<String, Integer> speciesIndexPair2totalOrthologsAll_;// total orthologs in all segments
		...
		if(REMERGE_LINKED_SEGMENTS){
			...
			for(int i = 0; i<segmentList.size(); ++i){
				...
				for(int j = 0; j<i; ++j){
					...
					if(species1.equals(species2)==false) continue;
					if(chr1.equals(chr2)){
	...
	static int getTotalOrthologsAll(Integer speciesIndex1, Integer speciesIndex2){
		...
		if(speciesIndex2.intValue()<speciesIndex1.intValue()) pair = speciesIndex2+"\t"+speciesIndex1;
	...
	static private void precomputeOrthologousBlocks(ArrayList<String> segmentList){
		...
		for(int i = 0; i<segmentList.size()-1; ++i){
			...
			for(int j = i+1; j<segmentList.size(); ++j){
				...
				if(index1.intValue()==index2.intValue()) continue;
				ConservedBlockList cbl2 = species2blockList_.get(species2);
				int blockSize2 = cbl2.getNumberOfGenes(seg2);
				int orthologCount = MultiSpeciesStatus.getOrthologCount(seg1, seg2);
				if(orthologCount==0) continue;
				int totalGenePairs = cbl1.getTotalNumberOfGenes()*cbl2.getTotalNumberOfGenes();
				if(totalGenePairs<0) throw new RuntimeException("integer overflow in precomputeOrthologousBlocks()");
				int n = getTotalOrthologsAll(index1, index2);// total orthologs in the genomes
				int m = totalGenePairs-n;// non ortholog gene pairs in the genomes
				int N = blockSize1*blockSize2;// chosen gene pairs
				int x = orthologCount;
		...
		Collections.sort(segmentList,
				new Comparator<ArrayList<String>>(){
					public int compare(ArrayList<String> ii1, ArrayList<String> ii2){
						...
						if(geneSize1<geneSize2) return 1;
						else if(geneSize1>geneSize2) return -1;
						else return 0;
					}
				}
				);
	...
	static int getOhnologPairSize(ArrayList<String> mergedSegments, ArrayList<String> group){
		...
		for(String seg1: mergedSegments){
			...
			for(String seg2: group){
				if(segment2speciesIndex(seg1)!=segment2speciesIndex(seg2)) continue;
		...
		Collections.sort(segmentList,
				new Comparator<ArrayList<String>>(){
					public int compare(ArrayList<String> ii1, ArrayList<String> ii2){
						...
						if(ohnologSize1<ohnologSize2) return 1;
						if(ohnologSize1>ohnologSize2) return -1;
						return 0;
					}
				}
	...
	public MultiSpeciesStatus computeReconstruction(PrintWriter logFile, boolean lastBatch) throws IOException{
		...
		if(this.segmentList_.size()>MAX_SEG_SIZE_FOR_SETPART){ return greedy; }
	...
	private MultiSpeciesStatus computeReconstruction(MultiSpeciesStatus status, MultiSpeciesStatus greedy, boolean lastBatch, PrintWriter logFile) throws IOException{
		MultiSpeciesStatus bestStatus = greedy;
		ArrayList<MultiSpeciesStatus> stack = new ArrayList<MultiSpeciesStatus>();
		stack.add(status);
		while(stack.isEmpty()==false){// depth-first search
			++timesEvaluated_;
	...
	public MultiSpeciesStatus resumeWithAdditionalSegments(MultiSpeciesStatus currentStatus, ArrayList<ArrayList<String>> additionalSegmentList, boolean lastBatch, PrintWriter logFile) throws IOException{
		...
		// precomputeParalogousAndOrthologousBlocks();
		...
		while(s.currentIndex_<segmentList_.size()-1){
			...
			for(MultiSpeciesStatus nextStatus: nextStatusList){
				...
				if(bestNextStatus.compareTo_partialScore(nextStatus)==1){
					// if(bestNextStatus.compareTo_bestScore(nextStatus)==1){
					bestNextStatus = nextStatus;
				}
			}
			s = bestNextStatus;
	...
	static private void printUsage(){
		...
		System.out.println("   -m INT  (minOhnologThreshold. segments are classified as chrUn if they have fewer ohnologs)");
		System.out.println("   -T template (=FREE_FORM/FOUR/PRE2R_FUSION/POST2R_FUSION/PRE2R_DOUBLE_FUSION or ALL to use all templates)");
		System.out.println("   -L FILE (log file)");
		System.out.println("   -I MAX_SEG_SIZE_FOR_SETPART (If segList.size()>MAX_SEG_SIZE_FOR_SETPART, set partitioning is skipped.)");
		...
		System.out.println("   -F INT  (minSpeciesCountForSubgroupOutput, This parameter filters subgroups or post-2R chromosomes comprising of segments from a limited number (<minSpeciesCountForSubgroupOutput) of species.)");
		System.out.println("   -C STRING (chromosome separator in the output reconstruction file. N for \\n and T for \\t.)");
	}
	public static void main(String[] args) throws IOException{
		...
		templateList[templateList.length-1] = null;// Excludes double fusion
		...
		for(int i = 0; i<orthologPairFile.size(); ++i){
			...
			while((line = br.readLine())!=null){
				...
				if(blockName1==null||blockName2==null) continue;
		...
		while((line = br.readLine())!=null){
			...
			for(int i = 0; i<data.length; ++i){
				if(SpeciesOhnologs.segment2speciesIndex(data[i])==null) continue;
				group.add(data[i]);
			}
			groupList.add(group);
		}
		...
		for(int g = 0; g<groupList.size(); ++g){
			...
			if(EXCLUDE_SEX_CHR){
				...
				for(String seg: segGroup){
					...
					if(chr1.equals("Y")||chr1.equals("W")) continue;
					newSegList.add(seg);
				}
				segGroup = newSegList;
			}
			...
			for(int t = 0; t<templateList.length; ++t){
				...
				if(allTemplateBestStatus==null){
					...
					if(bestStatus.compareTo_bestScore(allTemplateBestStatus)<0) allTemplateBestStatus = bestStatus;
				}
			}
