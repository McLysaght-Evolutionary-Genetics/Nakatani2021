...
public class PairedSpeciesStatus{
	...
	static TreeMap<String, Integer> speciesIndexPair2totalOrthologs_;// total orthologs in segmentList

	private void checkConsistentcy(){
		if(getTotalGenePairs()!=fixedGreenGenePairs_+fixedRedGenePairs_+remainingGenePairs_){ throw new RuntimeException("Consistency failure."); }
	}
	...
	public void updateStatus(int fixedRedGenePairsDelta, int fixedRedOrthologsDelta, int fixedGreenGenePairsDelta, int fixedGreenOrthologsDelta){
	...
	public double computePartialSignificance_logp(){
		if(Double.isNaN(partialSignificance_logp_)==false){ return partialSignificance_logp_; }
		...
		if(totalGenes1*totalGenes2==0) return 0;
		int n = getTotalOrthologs();// orthologs
		long m = getTotalGenePairs()-n;
		int N = this.fixedGreenGenePairs_;// gene pairs picked = inter-chr gene pairs
		int i = this.fixedGreenOrthologs_;// inter-chr duplicates

		// double lambda = N*n/(totalGenes1*totalGenes2);

		partialSignificance_logp_ = SpecialFunctions.logHypergeometricLowerProbability(n, m, N, i);
		if(partialSignificance_logp_>0) partialSignificance_logp_ = 0;
		return partialSignificance_logp_;
	}
	public double computeSignificanceLowerBound_logp(){
		if(Double.isNaN(significanceLowerBound_logp_)==false){ return significanceLowerBound_logp_; }
		long totalGenes1 = SpeciesOhnologs.getTotalGeneSize(species1_);
		long totalGenes2 = SpeciesOhnologs.getTotalGeneSize(species2_);
		if(totalGenes1*totalGenes2==0) return 0;
		int n = getTotalOrthologs();// ortholog
		long m = getTotalGenePairs()-n;
		int N = this.fixedRedGenePairs_+this.remainingOrthologs_;// gene pairs picked = inter-chr gene pairs
		int i = this.fixedRedOrthologs_+this.remainingOrthologs_;// inter-chr duplicates

		// double lambda = (totalGenes1*totalGenes2-N)*n/(totalGenes1*totalGenes2);
		// significanceLowerBound_logp_ = SpecialFunctions.logHypergeometricUpperProbability(n, m, N, i);
	...
	static public void countTotalGeneAndOhnologPairs(ArrayList<ArrayList<String>> segmentList){
		...
		for(int i = 0; i<flatSegmentList.size(); ++i){
			...
			for(int j = 0; j<i; ++j){
				...
				if(index2.intValue()<index1.intValue()) spPair = index2+"\t"+index1;
