...
public class SingleSpeciesStatus{
	...
	public void updateStatus(int fixedRedGenePairsDelta, int fixedRedOhnologsDelta, int fixedGreenGenePairsDelta, int fixedGreenOhnologsDelta){
	...
	public double computePartialSignificance_logp(){
		...
		int n = getTotalOhnologs();// duplicates
		long m = getTotalGenePairs()-n;// non-duplicates

		int N = this.fixedRedGenePairs_;// gene pairs picked = inter-chr gene pairs
		int i = this.fixedRedOhnologs_;// inter-chr duplicates
	...
	public double computeSignificanceLowerBound_logp(int currentIndex, ArrayList<ArrayList<String>> segmentList, int[] group){
		if(Double.isNaN(significanceLowerBound_logp_)==false){ return significanceLowerBound_logp_; }
		int n = getTotalOhnologs();// duplicates
		long m = getTotalGenePairs()-n;// non-duplicates
		int N = this.fixedRedGenePairs_+this.remainingOhnologs_;// gene pairs picked = inter-chr gene pairs
		int i = this.fixedRedOhnologs_+this.remainingOhnologs_;// inter-chr duplicates
		for(int j = currentIndex+1; j<group.length; ++j){
			...
			for(int x = 0; x<segList.size(); ++x){
				...
				if(index1.intValue()!=speciesIndex_) continue;
				...
				for(int y = 0; y<x; ++y){
					...
					if(index2.intValue()!=speciesIndex_) continue;
		...
		// double p2 = SpecialFunctions.logHypergeometricLowerProbability(n, m, n+m-N, n-i);
